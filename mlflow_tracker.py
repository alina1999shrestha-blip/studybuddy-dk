...
# Set MLflow tracking location — SQLite backend works on Windows and Docker
# Wrapped in try/except so Streamlit Cloud demo still works even if MLflow is unavailable
...
def log_pipeline_run_mlflow(student_input: dict, results: dict):
    ...
    try:
        ...
        print(f"⚠️ MLflow logging skipped (not available in this environment): {e}")
...
def log_monitoring_run_mlflow(monitoring_result: dict):
    ...
    try:
        ...
        print(f"⚠️ MLflow monitoring log skipped (not available in this environment): {e}")